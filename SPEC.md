...
Shinkansen-Nozomi 是一款瀏覽器擴充功能（Chrome & Firefox），將英文（或其他外語）網頁翻譯成繁體中文／簡體中文／日本語，協助使用者流暢閱讀外語內容。名稱取自新幹線最快車型「のぞみ（希望號）」，象徵快速、平穩、流暢的閱讀體驗。Fork 自 [jimmysu0309/shinkansen](https://github.com/jimmysu0309/shinkansen)，獨立發展。
...
滑鼠懸停顯示、原文樣式客製、輸入框翻譯、劃詞翻譯、DeepL / Yandex 等第三方付費翻譯服務、PDF/影片字幕、延遲載入、淺色/深色主題切換。

> 備注：v1.4.0 起已加入 Google Translate 非官方免費端點（Opt+G，不需 API Key），同時保留 Gemini（Opt+S）。Google 官方 Cloud Translation v2 API（付費）不在支援範圍內。
>
> v1.5.0 起已加入多語言介面（繁體中文 / 簡體中文 / 日本語），可在設定頁「一般設定 → 介面語言」切換。同版本起支援 Firefox（Manifest V3）。
...
完整預設 prompt 定義在 `lib/storage.js` 的 `DEFAULT_SYSTEM_PROMPT`（v0.83 升級）。v1.5 起，三語版本的提示詞定義在 `lib/i18n-prompts.js`，包含主翻譯、術語擷取、字幕翻譯三個 prompt 各自的 zh-TW / zh-CN / ja 版本。各語言版本針對目標語境重寫（例如簡體版使用大陸標準用語、日文版調整敬體/常體規則），而非簡單翻譯。使用者切換介面語言時，提示詞會透過 `getDefaultPromptsForLocale()` 自動切換。

繁體中文版採 XML tag 結構，分四大區塊：
...
`STATE.originalHTML`（Map，el → Node[] cloneChildSnapshot）備份每個被替換元素的原始子節點快照。再次按快速鍵呼叫 `restorePage()` 透過 `SK.restoreChildSnapshot()` 逐一還原。不使用 `innerHTML` 賦值（Mozilla AMO 合規要求）。
...
├── manifest.firefox.json      # Firefox MV3 manifest（v1.5: background.scripts、無 world:"MAIN"）
├── content-ns.js              # 命名空間、共用狀態 STATE、SK.t() 多語言函式
...
├── content-youtube-main-loader.js  # Firefox MAIN world 注入器（v1.5，構建時內嵌）
├── content-youtube.js         # YouTube 字幕翻譯（isolated world）
├── content.js                 # 主協調層（translatePage、Debug API、初始化）
├── content.css
├── background.js              # Service Worker（ES module）
├── lib/
│   ├── compat.js              # browser/chrome API 跨平台 Proxy shim
│   ├── i18n.js                # 多語言模組：t()、setLocale()、applyLocale()（v1.5）
│   ├── i18n-prompts.js        # 三語翻譯提示詞（zh-TW/zh-CN/ja）（v1.5）
...
│   ├── model-pricing.js       # 模型計價對照表
│   ├── constants.js           # 共用常數
│   ├── format.js              # 共用格式化函式（formatBytes/formatTokens/formatUSD）
│   └── vendor/                # 第三方程式庫（Chart.js）
├── popup/
│   ├── popup.html
│   ├── popup.js               # ES module
│   └── popup.css
├── options/
│   ├── options.html           # data-i18n / data-i18n-html 多語言標記
│   ├── options.js             # ES module
│   └── options.css
├── _locales/
│   ├── zh_TW/messages.json    # 繁體中文（預設語系）
│   ├── zh_CN/messages.json    # 簡體中文（v1.5）
│   └── ja/messages.json       # 日本語（v1.5）
├── icons/
└── privacy-policy.html
scripts/
└── build.js                   # esbuild 雙平台構建腳本（v1.5）
```
...
3. **v1.6 選區過濾**：若 `window.getSelection()` 有非空選區，用 `Range.intersectsNode()` 過濾 units，只保留與選區有交集的段落。`STATE.translationScope` 設為 `'selection'`
4. `packBatches()` 依字元預算 + 段數上限打包成批次
5. 術語表前置流程（依文章長度決定策略）
6. `runWithConcurrency()` 平行送出批次，每批經 `TRANSLATE_BATCH` 訊息到 background
7. background 的 handler 查快取 → 未命中則走 Rate Limiter → 呼叫 Gemini API
8. 每批回來立即注入 DOM（`injectTranslation`），Toast 更新進度
9. 全部完成後顯示成功 Toast（含 token 數、費用、快取命中率）
10. **選區模式差異**：不啟動 `scheduleRescanForLateContent()`、不設置 `stickyTranslate`/`STICKY_SET`、SPA observer rescan 跳過（Content Guard 保留）
...
v1.4.12 起提供三組 preset 快捷鍵：

```json
"commands": {
  "translate-preset-1": { "suggested_key": { "default": "Alt+A", "mac": "Alt+A" } },
  "translate-preset-2": { "suggested_key": { "default": "Alt+S", "mac": "Alt+S" } },
  "translate-preset-3": { "suggested_key": { "default": "Alt+D", "mac": "Alt+D" } }
}
```
...
| 平台 | 方式 | API |
|------|------|-----|
| **Firefox** | 設定頁點擊快捷鍵徽章即時編輯 | `browser.commands.update({ name, shortcut })` |
| **Chrome** | 跳轉 `chrome://extensions/shortcuts` | 無 update API，唯讀 |

**Firefox 編輯流程**（v1.7 新增，`options.js`）：

1. 偵測 `moz-extension://` → 為 `preset-key-{1,2,3}` 加 `data-editable` 屬性
2. 點擊 → 進入編輯模式（`data-editing`，藍框提示）
3. `keydown` 捕獲：從 `e.code` 取實體按鍵（避免 macOS Option+A → `å` 問題）
4. macOS 修飾鍵映射：`e.metaKey` → `Command`、`e.ctrlKey` → `MacCtrl`
5. 組合字串格式：`Alt+A`、`Command+Shift+K` 等（Firefox commands API 格式）
6. `browser.commands.update()` 即時生效並持久化，不需按「儲存設定」
7. 無效組合顯示錯誤 1.5 秒後恢復；`Esc` 或失焦取消編輯
...
- Header：emoji 🚄 + 名稱「Shinkansen-Nozomi」+ 版本號（動態讀取）
...
`background.js` 維護 `stickyTabs: Map<tabId, slot>`（v1.4.12 起 value 為 preset slot number），持久化於 `sessionStore`（Chrome 用 `storage.session`、Firefox 用 `storage.local` + `_sk_session_` 前綴 fallback，啟動時 `clearOnStartup` 清除殘留）。
...
- manifest commands：`translate-preset-1`（Alt+A 預設 Flash Lite）/ `translate-preset-2`（Alt+S 預設 Flash）/ `translate-preset-3`（Alt+D 預設 Google MT）。Firefox 可透過設定頁內的 `browser.commands.update()` 直接修改鍵位。
...
- **ES module 頁面**（popup / options / background）：`import { t } from './lib/i18n.js'`，透過 `t('key', ...args)` 取得翻譯字串
- **Content script**（不支援 import）：`content-ns.js` 內嵌三語字串表，透過 `SK.t('key', ...args)` 取得翻譯
- **HTML 靜態文字**：`data-i18n="key"` 屬性由 `applyLocale()` 自動替換 `textContent`；含 HTML 標籤的元素（連結、粗體等）由 `applyRichTextLocale()` 以 DOM API 安全構建（不使用 `innerHTML`，符合 Mozilla AMO 合規要求）
- **翻譯提示詞**：`lib/i18n-prompts.js` 定義三語版本的主翻譯 / 術語擷取 / 字幕翻譯提示詞，各語言針對目標語境重寫
- **語言設定**：存於 `chrome.storage.sync` 的 `uiLocale` 欄位，在設定頁「一般設定 → 介面語言」切換

### 17.3 字串表規模

三語各約 310 個 key，分類前綴：
- `popup_*`：Popup 面板（~30 keys）
- `opt_*`：設定頁（~180 keys）
- `cs_*`：Content script 翻譯進度/錯誤/完成（~35 keys）
- `yt_*`：YouTube 字幕（~10 keys）
...
使用 esbuild 打包，一套程式碼產出 Chrome 與 Firefox 兩個版本：

```bash
npm run build:chrome    # → build/chrome/
npm run build:firefox   # → build/firefox/
npm run build:all       # 同時構建兩個平台
```
...
| Background 格式 | ESM（`type: "module"`） | IIFE（`scripts: [...]`） |
| YouTube MAIN world | `world: "MAIN"` manifest 宣告 | `content-youtube-main-loader.js` 內嵌注入 |
| HTML `<script>` | `type="module"` | 去掉 `type` 屬性 |
...
1. 更新 `manifest.json` 與 `manifest.firefox.json` 的 version
2. 執行 `./release.sh "描述"` — 自動 `build:all`、commit、tag、push
3. GitHub Actions（`.github/workflows/release.yml`）自動構建並產出 chrome / firefox 兩個 zip
