...
- 文件版本：v1.0
- 建立日期：2026-04-08
- 最後更新：2026-04-19（v1.4.11）
- 目標平台：Chrome（Manifest V3）
- 作業系統：macOS 26
- 目前 Extension 版本：1.4.11
...
| `CLEAR_BADGE` | — | `{ ok }` |
| `STICKY_QUERY` | — | `{ ok, shouldTranslate, engine }` — 查當前 tab 是否在跨 tab sticky set 中（v1.4.11） |
| `STICKY_SET` | `{ engine: 'gemini' \| 'google' }` | `{ ok }` — 翻譯成功後將當前 tab 加入 sticky set（v1.4.11） |
...
### 14.5 跨 tab sticky 翻譯（v1.4.11）

`background.js` 維護 `stickyTabs: Map<tabId, engine>`，持久化於 `chrome.storage.session.stickyTabs`（service worker 休眠重啟時 hydrate 回 memory）。

行為：
- 任一 tab 翻譯成功（Gemini 或 Google）→ content.js 送 `STICKY_SET` → 該 tab 進入 set。
- `chrome.tabs.onCreated`：若新 tab 的 `openerTabId` 在 set 中，把新 tab 也加入 set 並繼承相同 engine。涵蓋 Cmd+Click、`target="_blank"`、`window.open()` 等所有由瀏覽器標記 opener 的開法。
- content script 載入時送 `STICKY_QUERY`，若回 `shouldTranslate=true` 自動觸發對應 engine 翻譯（優先順序：sticky > whitelist autoTranslate）。
