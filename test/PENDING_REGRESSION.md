...
### ~~vBulletin td.alt1 翻譯後標題 div 消失~~ — 已修復 → `test/regression/inject-vbulletin-title-div.spec.js`（v1.4.14）
（根因在 detection：DIV 不在 `BLOCK_TAGS_SET` 所以 `containsBlockDescendant(td)=false`，td 被 FILTER_ACCEPT 整段翻譯，injection 走 media-preserving path 清掉 smallfont。修法是結構性通則：block 元素若有直接 `CONTAINER_TAGS` 子元素且有候選文字 → FILTER_SKIP 讓 walker 下探；另新增 Case C 收集「只含 preservable inline、無 BR、無 direct text」的 wrapper div（如 smallfont 裡的 STRONG 標題）。injection path 不動。SANITY 通過：移除 guard 後 td 重新整段被收、smallfont 被清掉、斷言 fail；還原後 pass。）
