...
> [下載最新版本 zip](https://github.com/r404r/shinkansen/releases/latest) · 安裝教學與產品介紹見 [官方網頁](https://jimmysu0309.github.io/shinkansen/) · [功能變更紀錄](CHANGELOG.md)
...
- **選區翻譯**（Nozomi v1.6 新增）：選取頁面上的部分文字後按快速鍵，只翻譯選取範圍內的段落。適合只想看某一段的翻譯、不想翻整頁的場景
...
4. 選擇本專案的 `shinkansen/` 資料夾（或 `build/chrome/` 構建產物）
5. 擴充功能清單會出現 Shinkansen，可以固定到工具列

**Firefox — 開發版（暫時載入）**

1. 開啟 Firefox，網址列輸入 `about:debugging#/runtime/this-firefox`
2. 點「載入暫時性附加元件」（Load Temporary Add-on）
3. 選擇 `build/firefox/manifest.json`（需先執行 `npm run build:firefox`）
4. 注意：暫時載入的擴充功能在 Firefox 重啟後會消失

## 首次設定

1. 申請 Gemini API Key — 詳細步驟見 [API Key 申請教學](API-KEY-SETUP.md)
2. 點工具列的 Shinkansen-Nozomi 圖示 → 「設定」
...
Shinkansen-Nozomi 支援三種介面語言，翻譯的目標語言和提示詞會同時切換：

| 語言             | 翻譯目標     | 提示詞語境                       |
| ---------------- | ------------ | -------------------------------- |
| 繁體中文（預設） | 台灣繁體中文 | 台灣用語、台灣通行譯名           |
| 简体中文         | 大陆简体中文 | 大陆标准用语、大陆通行译名       |
...
- **翻譯快速鍵**（v1.4.12 起三組）：
  ...
  - 已翻譯狀態下按任一快速鍵 → 還原原文
  - 翻譯中按任一快速鍵 → 取消翻譯
- **選區翻譯**（Nozomi v1.6 新增）：用滑鼠選取頁面上的部分文字後按快速鍵，只翻譯選取範圍內的段落，不翻譯整頁。翻譯通知會顯示 `[選取翻譯]` 前綴。沒有選取文字時按快速鍵則正常翻譯整頁
...
- **Firefox**（Nozomi v1.7 新增）：直接在設定頁點擊快捷鍵徽章即可進入編輯模式，按下新的鍵組合即時生效。支援 `Alt+`、`Ctrl+`、`Shift+` 及 macOS 的 `Command+` 修飾鍵。按 `Esc` 取消編輯。
...
2. Shinkansen-Nozomi 會自動在新分頁開啟同一份文件的「行動版閱讀模式」（mobilebasic）
...
開啟有英文字幕的 YouTube 影片，確認 CC 已開啟，點工具列的 Shinkansen-Nozomi 圖示，Popup 會出現「YouTube 字幕翻譯」開關，打開即可。字幕會在不影響影片播放的情況下逐段替換成繁體中文，樣式與 YouTube 原生字幕完全一致。

若你常看 YouTube 英文影片，可在設定頁的「YouTube 字幕」Tab 開啟自動翻譯，進入影片頁面後字幕翻譯會自動啟動，不需每次手動開關。

### AI 智慧分句（upstream v1.7 起，自動產生字幕專用）

YouTube 自動產生字幕（沒有人工字幕的影片，CC 標記為 auto-generated）原本是「**按時間切割**」而非「按句子切割」——每條字幕只有 1-3 個英文字、沒有標點，逐條翻譯完全失去語意上下文，譯文會像被剁碎一樣難讀。

Shinkansen-Nozomi（upstream v1.7 起）對自動產生字幕導入專用流程：
...
Shinkansen-Nozomi 有兩層快取機制，各自在不同階段省錢：
...
**第二層：Gemini implicit context cache**——Google 伺服器端自動做的，當連續請求的 prompt 前綴相同（例如 system prompt + 術語表）時，Gemini 會快取這段前綴，命中的 token 只收正常價格的 25%。這不需要使用者設定，Shinkansen-Nozomi 會自動從 API 回應中讀取命中數據。
...
固定術語表的優先級高於自動術語表一致化。翻譯時，固定術語表的指令會放在 system prompt 的最末端，LLM 會給予最高權重。修改術語表後不需要手動清快取，Shinkansen-Nozomi 會自動讓舊快取失效。
...
LLM 在翻譯長文時，前後文的人名、地名翻譯容易出現不一致（例如同一個人名前面翻「強森」、後面變成「約翰森」）。開啟「術語表一致化」後，Shinkansen-Nozomi 會先掃描全文建立專有名詞對照表，再讓後續翻譯遵循同一套譯名。
...
v1.9.0 — 完整功能清單與規格詳見 [SPEC.md](SPEC.md)。
