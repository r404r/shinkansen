...
**v1.4.14** — 修正 vBulletin 論壇貼文頁（例如 forum.miata.net）翻譯後標題 div 整個消失的問題。結構上 `<td class="alt1">` 並列包含 `<div class="smallfont"><strong>標題</strong></div>` + `<hr>` + `<div class="postbitcontrol2">內文 + inline <img></div>`；因為 DIV 不在 `BLOCK_TAGS_SET`，`containsBlockDescendant(td)` 回 false，walker 把整個 td 當一個翻譯單元 → `injectIntoTarget(td, frag)` 偵測到 td 內有 `<img>` 走 media-preserving path → empty-parent cleanup 把 smallfont 與 STRONG 一併移除。修法（只改 detection layer）：(1) `content-detect.js` collectParagraphs 的 BLOCK_TAGS_SET 分支新增 guard，若該 block 有直接 CONTAINER_TAGS 子元素且有候選文字（`hasContainerChildWithCandidateText`）→ `FILTER_SKIP` 讓 walker 下探；(2) 在非 block 分支新增 Case C：wrapper div 只含 preservable inline（STRONG/B/EM 等非 `<a>`、由 `hasNonAnchorPreservableInline` 判斷）、沒有 BR、沒有 direct text、沒有 block 後代但 innerText 有候選內容 → 整體當 element 單元；stats 計數 `inlineWrapperUnit` / `blockWithContainerChildren`。結果：標題 div 與內文 div 分別被收集、各自注入，彼此不干擾。injection path 完全不動。新增 regression spec `test/regression/inject-vbulletin-title-div.spec.js` + fixture，SANITY 通過：移除 guard 後 td 重新被整體收集、smallfont 被清掉，斷言 fail；還原後 pass。
